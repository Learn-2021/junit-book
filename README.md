...
# FIR[Self-validating]T 스스로 검증하는 코드
테스트는 기대하는 것이 무엇인지 단언하지 않으면 테스트가 아니다. 단위 테스트는 시간을 소모하기보다 절약한다. 테스트 결과를 수동으로 검증하는 것은 소모적인 절차다. 테스트 코드는 스스로 검증 가능할 뿐만 아니라 준비할 수도 있어야 한다. 테스트를 실행하기 전에 수동으로 준비 단계를 만드는 것은 올바른 테스트 코드가 아니다. 테스트에 필요한 어떤 설정 단계든 자동화되어야 한다. 그럼에도 불구하고 테스트를 실행하는 데 외부 설정이 필요하다면 FIRST 중에 I(고립)을 위반한 것이다.

테스트 코드는 전체 시스템을 위한 단위 테스트 묶음의 일부로 동작한다. 종종 수동으로 실행하지만, 한 단계 나아가면 언제 어떻게 실행할지도 자동화 할 수 있다.

# FIRS[Timeley]

단위 테스트는 언제라도 작성할 수 있다. 시스템에 있는 오래된 부분에 코드를 추가하고 거기서 단위 테스트를 시작할 수도 있다. 가능하면 적절한 순간에 단위 테스트에 집중하는 것이 낫다. 단위 테스트는 좋은 습관이다.

# 무엇을 테스트할 것인가?

메서드 혹은 클래스 코드를 보았을 때 숨어 있는 모든 버그를 찾아내는 것은 불가능하다. 따라서 무엇을 어떻게 테스트할 것이 중요한지 판단할 지침이 필요하다.

Right-BICEP이라는 지침이 있다.

- Right : 결과가 올바른가?
- B: 경계 조건boundary conditions은 맞는가?
- I : 역 관계inversion relationship를 검사할 수 있는가?
- C : 다른 수단을 활용하여 교차 검사cross-check할 수 있는가?
- E : 오류 조건error conditions을 강제로 일어나게 할 수 있는가?
- P : 성능 조건performance characteristics은 기준에 부합하는가?
...
public void answerArithmeticMeanOfTowNumbers() {
	...
	assertThat(actualResult, equalsTo(6));
}
```

간단한 산술 평균 테스트다. 이 테스트는  `ScoreCollection` 클래스가 5와 7을 입력 받았을 때 평균으로 6을 반환하는지 테스트한다.

작성한 코드가 정상적으로 동작하는지 어떻게 알 수 있을까? 테스트를 작성하는 것이다. 단위테스트는 내가 작성한 코드가 어떤 일을 하고 어떤 결과를 반환하는지, 그리하여 작성한 코드가 제대로 작동하는지 보여준다.

이처럼 테스트의 궁극적인 목적은 코드의 동작 결과가 내가 예상하는 결과와 일치하는가?를 검증하는 것이다.
